...
module.exports = {
    ...
    run: async (interaction, client, mongodb) => {
        ...
        const [user] = ['user'].map(a => (interaction.fetchOption(a, true) ? interaction.fetchOption(a, true).value : (interaction.data.resolved) ? Object.keys(interaction.data.resolved['users'])[0] : null));//[interaction.fetchOption('prize', true), interaction.fetchOption]
