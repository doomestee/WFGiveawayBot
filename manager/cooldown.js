...
module.exports = class CooldownManager {
     ...
     */
    checkCooldown(id, command, time=Date.now(), type='user') {
        return (this._list[type][id]) ? (this._list[type][id][command]) ? (this._list[type][id][command] - time > 0) ? [false, this._list[type][id][command] - time] : [true, 0] : [true, -2, true] : [true, -1, true]; //this._list[type][id].reduce((acc, cur) => acc + cur, 0) : false;
    }
